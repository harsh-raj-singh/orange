...
from core.viz_api.routes import mcp_connect
...
def test_mcp_connect_mints_signed_token(monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        lambda credential: ("dev@example.com", "Dev User"),
    )

    client = TestClient(app)
    response = client.post("/mcp/connect", json={"credential": "google-id-token"})

    assert response.status_code == 200
    payload = response.json()
    assert payload["email"] == "dev@example.com"
    assert payload["name"] == "Dev User"
