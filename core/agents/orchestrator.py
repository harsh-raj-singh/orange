    ...
    *,
    ...
    async def _run_for_scope(
        *,
        ...
        solution_prompt = (
            SOLUTION_AGENT_GLOBAL_SCOPE_PROMPT if scope == "global" else SOLUTION_AGENT_USER_SCOPE_PROMPT
        )
        issue_prompt = ISSUE_AGENT_GLOBAL_SCOPE_PROMPT if scope == "global" else ISSUE_AGENT_USER_SCOPE_PROMPT
    ...
    if contribute_to_global:
        ...
        global_normalized = NormalizedSession(
        ...
        global_summary, global_errors = await _run_for_scope(
            scoped_session=global_normalized,
